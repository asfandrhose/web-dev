...
do -- Data Setup
	local cryIds = { 10840585584, 10840586720, 10840587401, 10840588430, 10840589223,--// Cries 1-5
		10840590127, 10840591115, 10840601554, 10840602654, 10840603347,--// Cries 6-10
		10840606887, 10840607586, 10840610122, 10840616398, 10840621794,--// Cries 11-15
		10840623121, 10840623968 }--// Cries 16 & 17
...
local cachedBattles = {}
local function completeBattle(num, gamemode, ch, isRandomizer)
	local b
	...
	local trainerDifficulty
	if gamemode == 'hardcore' then trainerDifficulty = math.random(1, 5) else trainerDifficulty = b.TrainerDifficulty end
	local trainerGender = b.TrainerGender
	if b.TrainerClass then
		local cd = data.Trainers.TrainerClassData[b.TrainerClass]
		local trainerPayout = b.Party[#b.Party].level * cd.BasePayout
		b.Payout = math.floor(gamemode == 'hardcore' and trainerPayout * 0.75 or trainerPayout)
		b.Gender = cd.Gender
	...
	for pindex, p in pairs(b.Party) do
		if isRandomizer then
			...
			p.moves = nil
			p.ability = nil
			if gamemode == 'shiny madness' then
				p.shiny = true
			end
		end
		if not p.id then
...
function Database:getBattle(num, dat)
	...
	if dat and table.find(gamemodes, dat.gamemode) then
		return completeBattle(num, dat.gamemode, nil, true)
	elseif not cachedBattles[num] then
		...
		return completeBattle(num, dat.gamemode)
	end
...
function DataService.checkchunk(player, chunkid)
	...
	local chunktoevent = {
		...
		['chunk150'] = 'EnterRoriaLeague',
		['chunk151'] = 'EnterRoriaLeague',
	...
	if chunktoevent[chunkid] then
		if chunktoevent[chunkid]:sub(1, 3) == 'gym' then
			if not PlayerData.badges[tonumber(chunktoevent[chunkid]:sub(4, 4))] then
				...
				pcall(function() player:Kick("Please avoid exploiting. Further explotation will result in a ban.") end)
				wait()
...
function DataService.fulfillRequest(player, ...)
	...
	for i, request in pairs({...}) do
		...
		if kind == 'Chunk' then
			...
			--DataService bs (after the folder request for thing)
			for _ , plr in (game:GetService("Players"):GetPlayers()) do                                                        
				local whitelist = {}

				if _f.PlayerDataService[plr].currentChunk == _f.PlayerDataService[player].currentChunk then
					if not table.find(whitelist, plr.Name) then -- so no clone
						table.insert(whitelist, plr.Name)
					end

					if not table.find(whitelist, player.Name) then -- so no clone
						table.insert(whitelist, player.Name) 
					end            
				end

				for _, oplr in ipairs(game:GetService("Players"):GetPlayers()) do
					if _f.PlayerDataService[oplr].currentChunk == _f.PlayerDataService[plr].currentChunk then
						if not table.find(whitelist, oplr.Name) then -- so no clone
							table.insert(whitelist, oplr.Name)
						end
					end
				end

				if #whitelist > 0 then
					_f.Network:post('HidePlayers', plr, whitelist)
				end
			...
			local function checkForBattles(obj)
				for _, ch in pairs(obj:GetChildren()) do
					if ch:FindFirstChild('#Battle') then
						...
						local battle = completeBattle(v, PlayerData.gamemode, ch)
						if battle then
