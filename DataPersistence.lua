...
return { -- todo: write something that checks matches between this list and the index list used by PDS:[de/]serialize
	...
	ChooseFirstPokemon = function(PlayerData, name)
		...
		if not table.find(gamemodes, PlayerData.gamemode) then
		...
		local starter = PlayerData:newPokemon {
			...
			level = 5,
			shinyChance = 1024,
			shiny = PlayerData.gamemode == 'shiny madness' and true or false,
			untradable = true,
		}
	...
	EnterRoriaLeague = function(PlayerData)
		if not PlayerData.badges[8] then return end 
	end,
	findMazeKey = function(PlayerData)
		if not PlayerData.badges[8] then return false end	
	end,
	completeMazePuzzle = function(PlayerData)
		if not PlayerData.badges[8] then return false end		
	end,
	TapuKoko = function(PlayerData)
		if not PlayerData.badges[8] then return false end
	end,
	MeetScaleBrother = "vPortDecca",
	MelemeleTicket = function(PlayerData)
		if not PlayerData.badges[8] then return false end 
		PlayerData:addBagItems({id = 'melemeleticket', quantity = 1})
	end,
	UlaulaTicket = function(PlayerData)
		if not PlayerData.badges[8] then return false end
		PlayerData:addBagItems({id = 'ulaulaticket', quantity = 1})
	end,
	...
	ObtainWing = function(PlayerData)
		PlayerData:addBagItems({id = 'rainbowwing', quantity = 1})
	end,
	HoOh = manualOnly,
	roriaLeagueTess = "EnterRoriaLeague",
	defeatGaia = "roriaLeagueTess",
	defeatMerigold = "defeatGaia",
	defeatWyvern = "defeatMerigold",
	defeatZara = "defeatWyvern",
	roriaChampion = "defeatZara",
	getRevealGlass = function(PlayerData)
		if not PlayerData.badges[5] then return false end
		PlayerData:addBagItems({id = 'revealglass', quantity = 1})
	end,
	meteoriteQuest = function(PlayerData)
		if not PlayerData.badges[8] then return false end
	end,
	Shard1 = manualOnly,
	Shard2 = manualOnly,
